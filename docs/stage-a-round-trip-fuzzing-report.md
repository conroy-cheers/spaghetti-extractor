...
Current machine-readable qualification evidence is generated by
`nix build .#stage-a-roundtrip-promoted-check --no-link`. Reports are Nix
outputs rather than checked-in documentation snapshots.

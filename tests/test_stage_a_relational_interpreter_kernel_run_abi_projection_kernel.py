...
class RunABIProjectionKernelTests(unittest.TestCase):
    def test_run_abi_projection_checks_without_bad_axioms(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            _copy_module_closure(
                source_root,
                stage_a,
                "RelationalInterpreterKernelRunABIProjection",
            )
            result = _run_lean_relational(
                root,
                bundle="RelationalInterpreterKernelRunABIProjection",
            )
        ...
        for theorem in (
            "ABIRequestFacts.runFunctionEntryEsp",
            "ABIRequestFacts.runFunctionInputWord",
            "ABIRequestFacts.runFunctionEngineState",
        ):

...
from spaghetti_extractor.components.qualification import (
...
class ComponentIntentV2Tests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_alternative_groups_resolve_but_overlapping_configuration_fails(self) -> None:
        ...
        parsed = load_component_catalog_intent_v2(self.intent)
    ...
    def test_split_and_grouped_configurations_have_exact_ownership(self) -> None:
        result = resolve_component_catalog_v2(
            proposals=self.proposals,
            intent=self.intent,
            out=self.root / "out.json",
        )
    ...
    def test_generated_fields_are_rejected_from_authored_intent(self) -> None:
        ...
        with self.assertRaisesRegex(ComponentIntentError, "generated fields"):
            load_component_catalog_intent_v2(self.intent)
    ...
    def test_group_contract_hides_internal_edges_and_requires_review(self) -> None:
        resolution = resolve_component_catalog_v2(
            proposals=self.proposals,
            intent=self.intent,
            out=self.root / "resolution.json",
        )
        draft = build_lift_unit_contract_v2(
        ...
        checked = build_lift_unit_contract_v2(
            machine_ir=self.machine,
            reconstruction_plan=self.plan,
            resolution=resolution,
            lift_unit_id="all",
    ...
    def test_review_cannot_hide_a_machine_effect(self) -> None:
        ...
        result = build_lift_unit_contract_v2(
            machine_ir=self.machine,
            reconstruction_plan=self.plan,
            resolution=resolution,
            lift_unit_id="all",
    ...
    def test_enabled_group_activates_only_after_exact_qualification(self) -> None:
        ...
        contract = build_lift_unit_contract_v2(
            machine_ir=self.machine,
            reconstruction_plan=self.plan,
            resolution=resolution,
            lift_unit_id="all",
        ...
        implementation = build_component_source_package_v2(
            lift_unit_id="all",
        ...
        evidence = bind_component_evidence_v2(
            {
                "format": COMPONENT_EVIDENCE_V2_FORMAT,
                "status": "satisfied",
                "lift_unit_id": "all",
        ...
        self.assertEqual(plan["format"], COMPONENT_ACTIVATION_PLAN_V2_FORMAT)
        self.assertEqual(plan["status"], "checked")
        self.assertEqual(plan["counts"]["portable_replacements"], 2)
        self.assertEqual(plan["counts"]["machine_ir_fallback"], 0)

    def test_source_change_invalidates_qualification_and_restores_fallback(self) -> None:
        payload = json.loads(self.intent.read_text(encoding="utf-8"))
        payload["configurations"][1]["selections"][0]["activation"] = "enabled"
        self.intent.write_text(json.dumps(payload), encoding="utf-8")
        resolution = resolve_component_catalog_v2(
        ...
        original = build_component_source_package_v2(
            lift_unit_id="all",
            files={"a.c": self.root / "a.c", "b.c": self.root / "b.c"},
            shared_inputs={},
            entry={"abi": "logical-c-v1", "symbol": "a"},
        ...
        changed = build_component_source_package_v2(
            lift_unit_id="all",
            files={"a.c": self.root / "a.c", "b.c": self.root / "b.c"},
            shared_inputs={},
            entry={"abi": "logical-c-v1", "symbol": "a"},
        ...
        qualification = qualify_lift_unit_v2(
            contract=contract,
            implementation=self.root / "source-changed",
            evidence=evidence,
            out=self.root / "qualification-stale.json",
        )
        self.assertEqual(qualification["status"], "violated")
        self.assertIn(
            "component_evidence_implementation_stale",
            {row["code"] for row in qualification["issues"]},
        )
        plan = compose_component_configuration_v2(
            machine_ir=self.machine,
            resolution=resolution,
        ...
        self.assertEqual(plan["counts"]["portable_replacements"], 0)
    ...
    def test_source_package_loader_rejects_tampered_or_unlisted_files(self) -> None:
        ...
        with self.assertRaisesRegex(ComponentIntentError, "hash is stale"):
            load_component_source_package_v2(package)
    ...
    def test_missing_enabled_qualification_retains_total_fallback(self) -> None:
        payload = json.loads(self.intent.read_text(encoding="utf-8"))
        payload["configurations"][1]["selections"][0]["activation"] = "enabled"
        self.intent.write_text(json.dumps(payload), encoding="utf-8")
        resolution = resolve_component_catalog_v2(

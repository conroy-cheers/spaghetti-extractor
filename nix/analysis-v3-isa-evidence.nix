...
let
  ...
  projectionPythonSource = import ./python-module-closure.nix {
    inherit pkgs;
    source = pythonSource;
    modules = [ "spaghetti_extractor.stage_a_isa_evidence_v3" ];
  ...
  projection = pkgs.runCommand "${name}-projection" (
    ...
    ${pythonEnv}/bin/python3 -m spaghetti_extractor.stage_a_isa_evidence_v3 \
      --requirements ${requirements}/requirements.json \
  ...
  frontiers = import ./analysis-v3-isa-frontiers.nix {

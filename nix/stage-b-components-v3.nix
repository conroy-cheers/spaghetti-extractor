...
let
  ...
  mkPhaseSource = phase: modules: import ./python-module-closure.nix {
    ...
    source = pythonSource;
    name = "${namePrefix}-components-v3-${phase}-python-closure";
  };
  ...
  mkActivationPlan = configuration:
    ...
    in pkgs.runCommand "${namePrefix}-${configuration.id}-component-activation-plan-v3" common ''
      ...
      import pathlib
      import sys
      from spaghetti_extractor.components.configuration import compose_component_configuration_v3

      compose_component_configuration_v3(
          machine_ir=pathlib.Path(sys.argv[1]),

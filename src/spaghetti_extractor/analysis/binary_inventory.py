...
def _raw_base_relocations(parsed: Any) -> list[dict[str, int]]:
    """Return the exact PE relocation inventory used by static provenance."""

    return [
        {"rva": int(entry.rva), "type": int(entry.type)}
        for block in getattr(parsed.pe, "DIRECTORY_ENTRY_BASERELOC", []) or []

...
SIDE_EXTRACTION_REQUEST_FORMAT = (
    "stage-a-relational-side-extraction-request-v1"
)
SIDE_EXTRACTION_RESULT_FORMAT = "stage-a-relational-side-extraction-v1"
...
class SideExtractionRequest:
    ...
    profile: str = STAGE_A_RELATIONAL_PROFILE_ID
...
def parse_request(payload: Any) -> SideExtractionRequest:
    ...
    if request["profile"] != STAGE_A_RELATIONAL_PROFILE_ID:
        raise StageAInputError("side extraction request profile mismatch")
    if request["model"] != STAGE_A_RELATIONAL_MODEL_ID:
...
def parse_result(
    ...
    *,
    ...
    if result["profile"] != STAGE_A_RELATIONAL_PROFILE_ID:
        raise StageAInputError("side extraction result profile mismatch")
    if result["model"] != STAGE_A_RELATIONAL_MODEL_ID:

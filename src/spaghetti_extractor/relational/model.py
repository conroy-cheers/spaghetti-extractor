...
def _stack_window_transfer_claims(
    ...
    def adjustment(expression: Any, register: str) -> dict[str, Any] | None:
        input_register = {"op": "input_reg", "reg": register}
        if expression == input_register:
        ...
        left = expression.get("left") or {}
        right = expression.get("right") or {}
        if left != input_register or right.get("op") != "constant":
            return None
        value = int(right.get("value", -1))
        if not 0 <= value < 2**32:
            return None
        word_offset = (value if operation == "add" else -value) % 2**32
        delta = word_offset if word_offset < 2**31 else word_offset - 2**32
        if not -(2**31) < delta < 2**31 or delta % 4 != 0:
            return None
        if delta == 0:
            return {"kind": operation, "amount": 0}
        if operation == "sub" and delta > 0:
            return None
        return {

"""Generate exact decoded-candidate/native-candidate adapter assembly.

The generated Lean module constructs checked artifact/kernel evidence, the
canonical decoded candidate view, the remaining local component premises, and
the final finite-chunk adapter.  It accepts no status or whole-program
equivalence field.
"""
...
INTERPRETER_EXACT_DECODED_NATIVE_ADAPTER_MODULE = (
    "GeneratedRelationalInterpreterExactDecodedNativeAdapter"
)
...
class InterpreterExactDecodedNativeAdapterGenerationError(StageAInputError):
    """A submitted module or Lean proof-term name is malformed."""


@dataclass(frozen=True)
class InterpreterExactDecodedNativeAdapterSpec:
    """Checked Lean names needed to assemble one finite-chunk adapter."""
    ...
    launch_chunk: str
    kernel_chunk: str
    external_boundary_chunk: str
    x87_replay_chunk: str
    ...
    def validate(self) -> None:
        if _STAGE_A_MODULE.fullmatch(self.binding_module) is None:
            raise InterpreterExactDecodedNativeAdapterGenerationError(
...
def relational_interpreter_exact_decoded_native_adapter_source(
    spec: InterpreterExactDecodedNativeAdapterSpec,
) -> str:
    """Emit checked kernel evidence, component premises, and chunk adapter."""
    ...
    return f"""import StageA.RelationalInterpreterExactDecodedNativeAdapter
...
def {spec.premises_name} :
  ...
  classify := {spec.classifier}
  rootsRelated := {spec.roots_related}
  launchChunk := {spec.launch_chunk}
  kernelChunk := {spec.kernel_chunk}
  externalBoundaryChunk := {spec.external_boundary_chunk}
  x87ReplayChunk := {spec.x87_replay_chunk}
}}
...
def write_relational_interpreter_exact_decoded_native_adapter(
    out: Path | str, spec: InterpreterExactDecodedNativeAdapterSpec
) -> Path:
    ...
    destination = (
        stage_a / f"{INTERPRETER_EXACT_DECODED_NATIVE_ADAPTER_MODULE}.lean"
    )
    destination.write_text(
        relational_interpreter_exact_decoded_native_adapter_source(spec),
        encoding="utf-8",
    )
    return destination

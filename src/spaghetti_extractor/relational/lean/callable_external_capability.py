...
This format intentionally contains no submitted execution trace. Runtime
identities, arguments, order, and global indices are produced only by the Lean
operational wrapper in ``RelationalCallableExternalExecution``.
"""
...
CALLABLE_EXTERNAL_CAPABILITY_FORMAT = (
    "stage-a-relational-callable-external-capability-v3"
)
...
def relational_callable_external_capability_source(payload: object) -> str:
    """Render static descriptors without any execution-trace authority."""
    artifact = parse_callable_external_capability_artifact(payload)
    definitions: list[str] = []
    resolver_names: list[str] = []
    capability_names: list[str] = []
    abi_names: list[str] = []

    for resolver in artifact.resolver_contracts:
        name = f"resolverCallContract{resolver.id}"
        resolver_names.append(name)
        definitions.append(
            f"def {name} : ResolverCallContract := {resolver.lean()}"
        )
    for capability in artifact.capabilities:
        capability_name = f"callableExternalCapability{capability.id}"
        capability_names.append(capability_name)
        definitions.append(
            f"def {capability_name} : CallableExternalCapability :=\n"
            f"  {capability.lean_capability()}"
        )
    for abi in artifact.resolved_abi_contracts:
        name = f"resolvedExternalABIContract{abi.id}"
        abi_names.append(name)
        definitions.append(
            f"def {name} : ResolvedExternalABIContract := {abi.lean()}"
        )

    checks = f"""
def resolverCallContracts : List ResolverCallContract := {_lean_list(resolver_names)}

def callableExternalCapabilities : List CallableExternalCapability :=
  {_lean_list(capability_names)}

def resolvedExternalABIContracts : List ResolvedExternalABIContract :=
  {_lean_list(abi_names)}

theorem resolverCallContractIdsUniqueChecked :
    resolverCallContractIdsUnique resolverCallContracts = true := by
  decide

theorem resolverCallContractShapesChecked :
    resolverCallContracts.all ResolverCallContract.shapeValid = true := by
  decide

theorem callableCapabilityIdsUniqueChecked :
    callableCapabilityIdsUnique callableExternalCapabilities = true := by
  decide

theorem callableCapabilityResourceIdsUniqueChecked :
    callableCapabilityResourceIdsUnique callableExternalCapabilities = true := by
  decide

theorem resolvedExternalABIContractIdsUniqueChecked :
    resolvedExternalABIContractIdsUnique resolvedExternalABIContracts = true := by
  decide

theorem resolvedExternalABIRoutesUniqueChecked :
    resolvedExternalABIRoutesUnique resolvedExternalABIContracts = true := by
  decide

theorem resolvedExternalABIContractShapesChecked :
    resolvedExternalABIContracts.all ResolvedExternalABIContract.shapeValid = true := by
  decide
""".strip()
    ...
    return (
        ...
        + "\n\n"
        + checks
        + "\n\nend StageA.GeneratedRelational.CallableExternalCapability\n"
    )

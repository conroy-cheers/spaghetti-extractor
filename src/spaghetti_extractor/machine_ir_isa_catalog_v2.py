...
def build_machine_ir_isa_catalog_proposal_v2(
    ...
    try:
        semantic_requirements = (
            build_machine_ir_isa_semantic_requirements_v2(requirements)
            if requirements.get("format") == MACHINE_IR_ISA_REQUIREMENTS_V2_FORMAT
            else parse_machine_ir_isa_semantic_requirements_v2(requirements)
        )
    ...
    binary_sha256 = _digest(
        semantic_requirements.get("binary_sha256"), "binary SHA-256"
    )
    checker = _mapping(
        semantic_requirements.get("checker"), "semantic requirements checker"
    )
    ...
    requirements_sha256 = _digest(
        semantic_requirements.get("semantic_requirements_sha256"),
        "semantic requirements SHA-256",
    )
    raw_forms = semantic_requirements.get("forms")
    ...
    for index, row in enumerate(raw_forms):
        ...
        form_id = _string(item.get("form_id"), f"form {index} ID")
        ...
        raw_encodings = item.get("instruction_hexes")
        if not isinstance(raw_encodings, list) or not raw_encodings:
            raise MachineIRISACatalogV2Error(
                f"form {index} contains no exact encodings"
            )
        for encoding_index, raw_encoding in enumerate(raw_encodings):
            encoded = _instruction_hex(
                raw_encoding, f"form {index} encoding {encoding_index}"
            )
            occurrence_id = "machine-ir-isa-semantic-occurrence-v2:" + canonical_sha256(
                {"form_id": form_id, "bytes": encoded}
            )
            encodings[(form_id, encoded)] = {
            ...
            "side_isa_artifacts": [
                {
                    ...
                    "artifact_sha256": requirements_sha256,
                }
            ],
        },
        "forms": forms,

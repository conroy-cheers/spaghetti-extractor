"""Canonical dependency identities shared by Stage A authority artifacts.

Analysis adapters may retain site-specific witnesses for diagnostics.  The
fixed-point authority graph, however, must compare the proof node which closes
the witness.  In particular, an exact internal call-frame witness is closed by
the checked summary of its callee.
"""

from __future__ import annotations

import json
from typing import Iterable


CALL_FRAME_DEPENDENCY_PREFIX = "call-frame:"
CALL_SUMMARY_NODE_PREFIX = "call-summary:"
CALL_FRAME_FAMILY_DEPENDENCY_PREFIX = "call-frame-family:"
CALL_SUMMARY_FAMILY_NODE_PREFIX = "call-summary-family:"

_CALL_SUMMARY_FAMILIES = frozenset({
    "memory",
    "register",
    "result",
    "return",
    "stack",
})


def call_frame_dependency_id(
    source_unit_id: str, event_index: int, target_unit_id: str
) -> str:
    payload = json.dumps(
        [source_unit_id, event_index, target_unit_id],
        separators=(",", ":"),
        ensure_ascii=True,
    )
    return f"{CALL_FRAME_DEPENDENCY_PREFIX}{payload}"


def parse_call_frame_dependency(
    value: object,
) -> tuple[str, int, str] | None:
    if not isinstance(value, str) or not value.startswith(
        CALL_FRAME_DEPENDENCY_PREFIX
    ):
        return None
    try:
        payload = json.loads(value.removeprefix(CALL_FRAME_DEPENDENCY_PREFIX))
    except (TypeError, ValueError, json.JSONDecodeError):
        return None
    if (
        not isinstance(payload, list)
        or len(payload) != 3
        or not isinstance(payload[0], str)
        or not payload[0]
        or not isinstance(payload[1], int)
        or isinstance(payload[1], bool)
        or payload[1] < 0
        or not isinstance(payload[2], str)
        or not payload[2]
    ):
        return None
    return payload[0], payload[1], payload[2]


def call_frame_family_dependency_id(
    source_unit_id: str,
    event_index: int,
    target_unit_id: str,
    family: str,
    subject: str | None = None,
) -> str:
    """Name one independently checked fact consumed at a call site."""

    _validate_call_family_parts(
        source_unit_id,
        event_index,
        target_unit_id,
        family,
        subject,
    )
    payload = json.dumps(
        [source_unit_id, event_index, target_unit_id, family, subject],
        separators=(",", ":"),
        ensure_ascii=True,
    )
    return f"{CALL_FRAME_FAMILY_DEPENDENCY_PREFIX}{payload}"


def parse_call_frame_family_dependency(
    value: object,
) -> tuple[str, int, str, str, str | None] | None:
    if not isinstance(value, str) or not value.startswith(
        CALL_FRAME_FAMILY_DEPENDENCY_PREFIX
    ):
        return None
    try:
        payload = json.loads(
            value.removeprefix(CALL_FRAME_FAMILY_DEPENDENCY_PREFIX)
        )
    except (TypeError, ValueError, json.JSONDecodeError):
        return None
    if not isinstance(payload, list) or len(payload) != 5:
        return None
    try:
        _validate_call_family_parts(
            payload[0], payload[1], payload[2], payload[3], payload[4]
        )
    except ValueError:
        return None
    return payload[0], payload[1], payload[2], payload[3], payload[4]


def call_summary_family_node_id(
    target_unit_id: str,
    family: str,
    subject: str | None = None,
) -> str:
    _validate_call_family_parts(
        "provider", 0, target_unit_id, family, subject
    )
    payload = json.dumps(
        [target_unit_id, family, subject],
        separators=(",", ":"),
        ensure_ascii=True,
    )
    return f"{CALL_SUMMARY_FAMILY_NODE_PREFIX}{payload}"


def _validate_call_family_parts(
    source_unit_id: object,
    event_index: object,
    target_unit_id: object,
    family: object,
    subject: object,
) -> None:
    if not isinstance(source_unit_id, str) or not source_unit_id:
        raise ValueError("call-frame family source unit must be non-empty")
    if (
        not isinstance(event_index, int)
        or isinstance(event_index, bool)
        or event_index < 0
    ):
        raise ValueError("call-frame family event index must be non-negative")
    if not isinstance(target_unit_id, str) or not target_unit_id:
        raise ValueError("call-frame family target unit must be non-empty")
    if not isinstance(family, str) or family not in _CALL_SUMMARY_FAMILIES:
        raise ValueError("call-frame family is unsupported")
    if subject is not None and (not isinstance(subject, str) or not subject):
        raise ValueError("call-frame family subject must be null or non-empty")
    if (family == "register") != (subject is not None):
        raise ValueError(
            "register call-frame facts require one subject; other families do not"
        )


def canonical_authority_dependency(value: str) -> str:
    """Return the checked fixed-point node which discharges ``value``.

    Unknown or malformed dependency families remain unchanged.  They then
    fail closed unless an authority node with that exact identity exists.
    """

    call_frame_family = parse_call_frame_family_dependency(value)
    if call_frame_family is not None:
        return call_summary_family_node_id(
            call_frame_family[2],
            call_frame_family[3],
            call_frame_family[4],
        )
    call_frame = parse_call_frame_dependency(value)
    return value if call_frame is None else f"{CALL_SUMMARY_NODE_PREFIX}{call_frame[2]}"


def canonical_authority_dependencies(values: Iterable[str]) -> tuple[str, ...]:
    return tuple(sorted({
        canonical_authority_dependency(value) for value in values
    }))


__all__ = [
    "CALL_FRAME_DEPENDENCY_PREFIX",
    "CALL_FRAME_FAMILY_DEPENDENCY_PREFIX",
    "CALL_SUMMARY_NODE_PREFIX",
    "CALL_SUMMARY_FAMILY_NODE_PREFIX",
    "call_frame_dependency_id",
    "call_frame_family_dependency_id",
    "call_summary_family_node_id",
    "canonical_authority_dependencies",
    "canonical_authority_dependency",
    "parse_call_frame_dependency",
    "parse_call_frame_family_dependency",
]

...
def create_reconstruction_workspace(
    *,
    ...
    workspace_payload = {
        "format": RECONSTRUCTION_WORKSPACE_FORMAT,
        "status": "editable",
        "executes_original_binary": False,
...
def rebind_reconstruction_workspace(*, workspace: Path) -> RegionReplacementManifest:
...
def check_reconstruction_workspace(
    *,
    workspace: Path,
    ...
    *, plan: Path, registry: Path | None, out: Path
    ...
    payload = {
        ...
        "interpretation": (
            ...
            "candidate-only behavioral evidence and does not authorize a formal whole-program pass."
        ),
    }
...
def _load_workspace(root: Path) -> Mapping[str, Any]:
    ...
    _require_format(payload, RECONSTRUCTION_WORKSPACE_FORMAT, "reconstruction workspace")
    return payload
